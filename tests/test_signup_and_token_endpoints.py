...
class TestUserRegistation:
    ...
    def test_singup_access(self, client):
        ...
        assert (
            response.status_code != HTTPStatus.NOT_FOUND
        ), f'Проверьте {self.URL_SIGNUP} в urls.py, т.к. запрос возвращает 404'
        assert (
            response.status_code != HTTPStatus.FORBIDDEN
        ), f'Убедитесь, что доступ эндпоинту {self.URL_SIGNUP} имеют все пользователи с валидными данными'
        assert (
            response.status_code == HTTPStatus.CREATED
        ), f'Убедитесь что запрос к эндпоинту {self.URL_SIGNUP} с валидными данными возвращает статус 200'
    ...
    def test_login_access(self, client):
        ...
        assert (
            response.status_code != HTTPStatus.NOT_FOUND
        ), f'Проверьте {self.URL_TOKEN} в urls.py, т.к. запрос возвращает 404'
        assert (
            response.status_code != HTTPStatus.FORBIDDEN
        ), f'Убедитесь, что доступ эндпоинту {self.URL_TOKEN} имеют все пользователи с валидными данными'
        assert (
            response.status_code == HTTPStatus.OK
        ), f'Убедитесь что запрос к эндпоинту {self.URL_TOKEN} с валидными данными возвращает статус 200'
    ...
    def test_logout_access(self, user_client):
        ...
        assert (
            response.status_code != HTTPStatus.NOT_FOUND
        ), f'Проверьте {self.URL_LOGOUT} в urls.py, т.к. запрос возвращает 404'
        assert (
            response.status_code != HTTPStatus.FORBIDDEN
        ), f'Убедитесь, что доступ эндпоинту {self.URL_LOGOUT} имеют авторизированные пользователи'
        assert (
            response.status_code == HTTPStatus.NO_CONTENT
        ), f'Убедитесь что запрос к эндпоинту {self.URL_TOKEN} возвращает статус 204'
