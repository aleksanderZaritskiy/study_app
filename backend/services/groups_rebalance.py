    ...
    * метод rebalance самостоятельный метод реализиует сам процесс ребалансировки групп.
    ...
    def rebalance(self) -> None:
        ...
        if self.__need_to_rebalance:
            ...
            all_students = [
                student for group in self.groups for student in group.student.all()
            ]
            GroupStudents.objects.del_groups_related_course(self.course)

            if (
                len(self.groups) * self.max_group_size - len(all_students)
                >= self.max_group_size
            ):
                self._remove_group()
    ...
    def _create_group(self) -> Group:
        new_group = Group.objects.create(
            course=self.course, name=f"{self.course.name}, group #{len(self.groups) + 1}"
        )
